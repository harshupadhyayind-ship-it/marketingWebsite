        ...
        <TiltCard className="relative overflow-hidden rounded-2xl border border-[#1C1C1C]/08 cursor-pointer">
          ...
          <div className="relative z-10 p-10 md:p-14 flex flex-col justify-between h-full" style={{ minHeight: 460 }}>
            ...
            <div>
              ...
              <p className="text-[#1C1C1C]/50 text-sm leading-relaxed mt-3 max-w-lg">
                {project.description}
              </p>
...
export default function WorkPageContent() {
  ...
  return (
    <div className="bg-[#F5EFE6] min-h-screen">
      ...
      {/* ── Page header ── */}
      <section className="pt-36 pb-14 px-8 max-w-7xl mx-auto border-b border-[#1C1C1C]/08">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
          <div>
            ...
            >
              Work that
              <br />
              <em className="text-[#D64545] not-italic">speaks.</em>
            </h1>
          </div>
      ...
      {/* ── Filter bar ── */}
      <section className="sticky top-0 z-30 bg-[#F5EFE6]/95 backdrop-blur-md border-b border-[#1C1C1C]/08 px-8 py-5">
            ...
            {allTags.map((tag) => (
              <button
                ...
                className={`relative text-[11px] font-mono uppercase tracking-[0.18em] transition-colors duration-200 pb-1 ${
                  activeTag === tag
                    ? "text-[#D64545]"
                    : "text-[#1C1C1C]/35 hover:text-[#1C1C1C]/70"
                }`}
              >
                {tag}
                {activeTag === tag && (
                  <span className="absolute bottom-0 left-0 right-0 h-px bg-[#D64545]" />
                )}
              </button>
            ))}
          </div>
