    ...
    <group ref={groupRef} position={[0, 0, 0]}>
      ...
      <Float speed={1.2} floatIntensity={1} rotationIntensity={0.6}>
        <Sphere args={[0.6, 32, 32]} position={[3.2, 1.2, -1]}>
          <MeshDistortMaterial color="#B8C9A8" distort={0.3} speed={3} roughness={0.1} metalness={0.2} emissive="#B8C9A8" emissiveIntensity={0.25} transparent opacity={0.8} />
        </Sphere>
      </Float>
      <Float speed={1.5} floatIntensity={1.2} rotationIntensity={0.8}>
        <Sphere args={[0.45, 32, 32]} position={[-3, -1.2, -0.5]}>
          <MeshDistortMaterial color="#7A9068" distort={0.5} speed={4} roughness={0.05} metalness={0.1} emissive="#7A9068" emissiveIntensity={0.3} transparent opacity={0.75} />
        </Sphere>
      </Float>
      <Float speed={0.9} floatIntensity={0.9}>
        <Sphere args={[0.35, 32, 32]} position={[1.8, -2.5, -0.8]}>
          <MeshDistortMaterial color="#9CAF88" distort={0.4} speed={2.5} roughness={0.1} metalness={0.2} emissive="#9CAF88" emissiveIntensity={0.3} transparent opacity={0.7} />
        </Sphere>
      </Float>

      {/* Decorative rings */}
      <Ring position={[0, 0, 0]} rotation={[Math.PI / 2.5, 0.3, 0]} scale={3.5} color="#9CAF88" />
      <Ring position={[0, 0, 0]} rotation={[Math.PI / 3, 1.2, 0.5]} scale={5} color="#B8C9A8" />
      <Ring position={[2, 1, -2]} rotation={[0.5, 0.3, 0.2]} scale={2} color="#D4CAB8" />

      <pointLight position={[0, 3, 3]} intensity={4} color="#9CAF88" distance={15} />
      <pointLight position={[-4, -2, 2]} intensity={2} color="#F5F1E8" distance={12} />
    </group>
  );
}
